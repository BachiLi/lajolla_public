...
\paragraph{Emissive volumes.} Throughout the homework, we will assume the volume is not emissive (the emission always comes from the surfaces.)
...
We reach the final stage of this homework. This last part is a bit tough, so don't worry too much if you cannot finish it. Even if you don't finish this part, you will still get $90\%$ of the score. But the math is fun and you should try to understand it!
...
\paragraph{Homogenized free-flight sampling.} To resolve this, we apply a trick called \emph{homogenization}. That is, we convert the heterogeneous medium into a homogeneous medium by inserting \emph{fake} particles that does not scatter lights. Then we can use the closed form solution to obtain an answer. Mathematically, we modify the radiative transfer equation as follows:
\begin{equation}
...
\end{equation}
where $\sigma_n$ is the density of the \emph{fake} particles. All we did is to add $-\sigma_n + \sigma_n$ to the right hand side, so we do not alter the values of the radiative transfer equation at all.
...
numerical error can lead to unbounded loops.
2) We divide both transmittance and the PDF with the maximum of the majorant. This is to improve
numerical robustness. Note that whenever we hit the fake particle, both the transmittance and
the free-flight PDF needs to be multiplied with $\sigma_n$ ($\sigma_m (1 - \sigma_t / \sigma_m) = \sigma_n$),
...
average of the PDFs over the RGB channels. For example, when updating the path throughput:
\begin{lstlisting}
