...
Spectrum eval_op::operator()(const Lambertian &bsdf) const {
    ...
    return fmax(dot(dir_light, vertex.shading_frame.n), Real(0)) * 
           eval(bsdf.reflectance, vertex, ray_footprint, texture_pool) / c_PI;
}
Spectrum eval_op::operator()(const RoughPlastic &bsdf) const {
    ...
    Spectrum R = eval(
        bsdf.diffuse_reflectance, vertex, ray_footprint, texture_pool);
    Spectrum S = eval(
        bsdf.specular_reflectance, vertex, ray_footprint, texture_pool);
    Real roughness = eval(
        bsdf.roughness, vertex, ray_footprint, texture_pool);
    ...
    Real cos_theta_h = dot(half_vector, n);
    Real D = GTR2(cos_theta_h, roughness); // "Generalized Trowbridge Reitz", GTR2 is equivalent to GGX.
    Real G = smith_masking(cos_theta_v, roughness) * smith_masking(cos_theta_l, roughness);

    Spectrum spec_contrib = S * (F_l * D * G) / (4 * cos_theta_l * cos_theta_v);
...
Real pdf_sample_bsdf_op::operator()(const RoughPlastic &bsdf) const {
    ...
    Real cos_theta_in  = dir == TransportDirection::TO_LIGHT ? cos_theta_v : cos_theta_l;
    ...
    Real G = smith_masking(cos_theta_in, roughness);
    Vector3 half_vector = normalize(dir_light + dir_view);
    Real cos_theta_h = dot(half_vector, n);
    Real D = GTR2(cos_theta_h, roughness);
    spec_prob *= (G * D) / (4 * cos_theta_in);
...
std::optional<Vector3> sample_bsdf_op::operator()(const RoughPlastic &bsdf) const {
    ...
    if (u.x < spec_prob) {
        ...
        // Reflect the incoming direction over ellip_N (the micro normal)
        Vector3 reflected = -ellip_dir_in + 2 * dot(ellip_dir_in, ellip_N) * ellip_N;
...
Real get_roughness_op::operator()(const RoughPlastic &bsdf) const {
    return eval(bsdf.roughness, vertex, ray_footprint, texture_pool);
}
////////////////////////////////////////////////////////////////////////
